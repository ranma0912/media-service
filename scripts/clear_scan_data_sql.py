...
def clear_all_scan_data():
    ...
    try:
        with engine.connect() as conn:
            ...
            # 使用原生SQL查询统计信息
            media_files_count = conn.execute(text("SELECT COUNT(*) FROM media_files")).scalar()
            scan_paths_count = conn.execute(text("SELECT COUNT(*) FROM scan_paths")).scalar()
            scan_history_count = conn.execute(text("SELECT COUNT(*) FROM scan_history")).scalar()
            scan_progress_count = conn.execute(text("SELECT COUNT(*) FROM scan_progress")).scalar()
            file_tasks_count = conn.execute(text("SELECT COUNT(*) FROM file_tasks")).scalar()
            subtitle_files_count = conn.execute(text("SELECT COUNT(*) FROM subtitle_files")).scalar()
            ...
            try:
                ...
                conn.execute(text("DELETE FROM subtitle_files"))
                
                # 删除文件任务
                logger.info("正在删除文件任务...")
                conn.execute(text("DELETE FROM file_tasks"))
                
                # 删除扫描进度
                logger.info("正在删除扫描进度...")
                conn.execute(text("DELETE FROM scan_progress"))
                ...
                conn.execute(text("DELETE FROM scan_paths"))
